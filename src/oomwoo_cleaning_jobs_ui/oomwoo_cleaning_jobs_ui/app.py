    ...
    def mousePressEvent(self,event):
        ...
        try:
            if self.window.mode in ('create_rectangle', 'virtual_wall'):
                ...
                return
            # Two-point actions only need the map; paint/erase require generated
            # candidates and a selected Region.
...
class Window(QMainWindow):
    def __init__(self):
        ...
        self.advanced_toggle=QPushButton('Show Advanced Editing'); self.advanced_toggle.clicked.connect(self.toggle_advanced); left.addWidget(self.advanced_toggle)
        self.advanced=QGroupBox('Advanced Editing (use only when candidates are wrong)'); advanced_layout=QVBoxLayout(self.advanced)
        for title,fn in [('New Region',self.begin_rectangle),('Paint',lambda:self.set_mode('paint')),('Erase',lambda:self.set_mode('erase')),('Rename Current Region',self.rename),('Delete Region',self.delete),('Merge Into…',self.merge),('Split (Horizontal Cut)',self.split),('Add Keepout',self.add_keepout),('Add Virtual Wall',self.begin_wall),('Remove Constraint',self.remove_constraint)]:
        ...
        self.status=QLabel('Open a map.yaml or start /map'); left.addWidget(self.status); self.resize(1050,700)
    ...
    def set_mode(self,mode):
        self.mode=mode; self._first_point=None; self.status.setText('Mode: '+mode)
    def begin_rectangle(self):
        ...
        if ok and name.strip():
            ...
            self.status.setText('Click the rectangle top-left corner, then the bottom-right corner')
    def begin_wall(self):
        name,ok=QInputDialog.getText(self,'Virtual Wall','Name:')
        if not (ok and name.strip()): return
        width,ok=QInputDialog.getDouble(self,'Virtual Wall','Width (m):',0.05,0.001)
        if ok:
            self.mode='virtual_wall'; self._pending_name=name.strip(); self._pending_wall_width=width; self._first_point=None
            self.status.setText('Click the virtual wall start point, then the end point')
    def handle_two_point_action(self,row,col):
        if self._first_point is None:
            self._first_point=(row,col); self.status.setText('First point recorded; click the second point'); return
        ...
        self.status.setText('Virtual Wall added; Regions clipped immediately'); self.mode='paint'; self.refresh()
    ...
    def open_file(self):
        path,_=QFileDialog.getOpenFileName(self,'Open nav2 map.yaml','','YAML (*.yaml *.yml)')
        if path:
    ...
    def start_live(self):
        try:
            ...
            self._thread=threading.Thread(target=self._executor.spin,daemon=True);self._thread.start();self.status.setText('Subscribing to transient-local /map')
    ...
    def stop_live(self):
        ...
        self.status.setText('/map stopped')
    ...
    def replace_source(self, source):
        ...
        if current is not None and current.identity == source.identity:
            self.status.setText('Map identity unchanged; keeping current editing session')
            return False
        if current is not None:
            answer = QMessageBox.question(
                self, 'Map Changed',
                'Map identity changed; replacing the current session switches maps '
                'and does not migrate regions. Continue?',
    ...
    def name_candidates(self):
        if self.controller.regions is None:
            self.error('Run auto segmentation first'); return
        # Drop stale candidate labels deleted/merged outside the naming flow so
        # we never look up a name that no longer exists.
        ...
        while self._unnamed_candidates:
            ...
            name,ok=QInputDialog.getText(self,'Name Candidate Region',f'Name for candidate Region {label}:',text=current)
            if not ok:
                done=total-len(self._unnamed_candidates)
                self.status.setText(f'Named {done}/{total} candidates; you can resume naming later')
                return
            if not name.strip():
                self.error('Region name cannot be empty'); return
            self.controller.regions.rename(label,name.strip()); self._unnamed_candidates.remove(label); self.refresh()
        self.status.setText(f'All {total} candidates named; save draft or validate/publish')
    def _coordinates(self,prompt):
        text,ok=QInputDialog.getMultiLineText(self,'Coordinates',prompt)
        if not ok:return None
        return [tuple(map(float,line.replace(',',' ').split())) for line in text.splitlines() if line.strip()]
    def add_keepout(self):
        try:
            ident,ok=QInputDialog.getText(self,'Keepout','Name:'); points=self._coordinates('One map-frame x,y per line (at least three lines):') if ok else None
            if ident and points:self.controller.add_keepout(ident,points);self.status.setText('Keepout added; Regions clipped immediately');self.refresh()
        except Exception as exc:self.error(str(exc))
    def remove_constraint(self):
        ...
        if names:
            ident,ok=QInputDialog.getItem(self,'Remove Constraint','Constraint:',names,0,False)
            if ok:
                try:self.controller.remove_constraint(ident);self.status.setText('Constraint removed; clipped Region cells are not restored');self.refresh()
                except Exception as exc:self.error(str(exc))
    def save(self):
        try:self.controller.save_draft();self.status.setText('Draft saved')
        except Exception as exc:self.error(str(exc))
    def publish(self):
        try:
            report=self.controller.report();text='\n'.join(f'[{i.level}] {i.message}' for i in report.issues) or 'No issues'
            if report.errors:self.error(text);return
            self.controller.publish();QMessageBox.information(self,'Published',text)
        except Exception as exc:self.error(str(exc))
    def rename(self):
        label=self.selected_label()
        if label:
            name,ok=QInputDialog.getText(self,'Rename','Name:',text=self.controller.regions.names[label])
            if ok and name:
    ...
    def merge(self):
        ...
        if source:
            target,ok=QInputDialog.getInt(self,'Merge','Target label:',1)
            if ok:
                try:
    ...
    def split(self):
        label=self.selected_label()
        if label:
            row,ok=QInputDialog.getInt(self,'Split','Cut row:',self.controller.source.height//2,0,self.controller.source.height-1)
            if ok:
                cut=np.zeros(self.controller.source.cells.shape,dtype=bool);cut[row,:]=True
                pieces=self.controller.regions.split(label,cut)
                self.status.setText(f'Split into {len(pieces)} pieces' if pieces else 'Cut line did not split the Region into two pieces')
                self.refresh()
    def refresh(self):
        ...
        # PyQt5 does not accept NumPy's memoryview; pass bytes and copy
        # immediately so QImage never holds a temporary NumPy buffer.

...
Automatic partitioning is a replaceable ROS 2 module behind `oomwoo_segmentation_msgs/SegmentRooms`. This branch runs only the native engine shipped in `oomwoo_segmentation`; failures are explicit and never fall back to a legacy algorithm. The authoritative result is a source-grid `int32` label mask where 0 is unassigned and positive labels are Candidate Regions.
...
- `src/oomwoo_segmentation`: GPLv3 native room-segmentation engine (pure in-memory port of the pinned upstream ROSE + ROSE2 pipeline), ROS 2 action server and client, map model/I/O, canonical result validation, and rendering/CLI.
...
Implemented packages: `oomwoo_segmentation_msgs` provides the standard-type ROS 2 `SegmentRooms` action and room/wall messages; `oomwoo_segmentation` contains the GPLv3 native engine, wall extraction from retained extended segments, the ROS 2 action server and client, SourceMap identity/masks, Nav2 trinary loading, canonical result validation (labels plus Detected Walls), and rendering/CLI. `oomwoo_cleaning_jobs_core` contains `RegionSet` editing, constraints, validation, and persistence only.
...
All code, tests, interfaces, and development tools live in this repository; internal ROS 2/Python packages are split under `src/`. Cleaning domain logic remains in `oomwoo_cleaning_jobs_core`. The room-segmentation seam is owned by `oomwoo_segmentation_msgs`, and the native engine plus algorithm-neutral tooling by `oomwoo_segmentation`. Future cleaning-job actions/messages remain separate from segmentation interfaces.
